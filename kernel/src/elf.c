...
static uint64_t pte_flags_from_phdr(uint32_t p_flags) {
    uint64_t flags = 1ULL << 0; 
    if (p_flags & PF_W) flags |= (1ULL << 1); 
    return flags;
}
...
int elf_load(page_table_t *pml4, const void *elf_image, uint64_t *entry_out) {
    ...
    for (uint16_t i = 0; i < eh->e_phnum; i++) {
        ...
        //uint64_t flags  = pte_flags_from_phdr(seg->p_flags) | (1ULL << 1);  //testing part
	uint64_t flags  = pte_flags_from_phdr(seg->p_flags) ;
